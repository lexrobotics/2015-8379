...
void initUS(){
		servo[USfrontserv] = 210;
		servo[USbackserv] = 115;
}
...
//----------------------------------------------------------------
	...
	eraseDisplay();
	wait1Msec(2000);
	nxtDisplayCenteredTextLine(2, "%d", USreadDist(USback));
	wait1Msec(5000);

	if (abs(USreadDist(USfront) - USreadDist(USback)) < 5 && USreadDist(USfront) < 150){
		Cposition = 1;
		wait1Msec(1000);
		playSound(soundBeepBeep);
		wait1Msec(2000);
	}
	else if (abs(USreadDist(USfront) - USreadDist(USback)) < 70 && USreadDist(USfront) < 150){
		Cposition  = 2;
		wait1Msec(1000);
