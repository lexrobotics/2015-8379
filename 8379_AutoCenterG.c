...
void initUS()
{
	servo[USfrontservo] = 119;
	servo[USbackservo] = 110;
}
void armOut(){
	time1[T3]=0;
...
{
	nxtDisplayCenteredTextLine(2, "%d, %d", USReadDist(USback), SensorValue(USfront));
	mecJustMove(speed, degrees, speedRotation);
	if (till){
		while ((SensorValue(USfront) > threshold)){}}
	else{
		while ((USreadDist(USback)) < threshold || (SensorValue(USfront) < threshold)){}}//should be ||, so stop when none of them is in the threshold
	Stop();
	//mecMove(80, 0, 0, 23.5);
}
...
void parallel()
{
float difference=USreadDist(USback) > SensorValue(USfront)? 20:-20;//so that the sensors doesn't have to detect twice; to save batteries
	mecJustMove(0, 0, difference);
	while(abs(USreadDist(USback)-SensorValue(USfront))>5)
	{}
	Stop();
}
...
//**the function can stop and alarm when it is not aligning anymore, which is better than alignT()
{
	...
	if (TSreadState(TOUCHfront) == 1 || TSreadState(TOUCHback) == 1)//run if at least one of them is touching, else... it is just unfortunate
	{
		nxtDisplayCenteredTextLine(2, "%d, %d", TSreadState(TOUCHfront), TSreadState(TOUCHback));
		mecMove(10, 90, 0, 2);
		const short reading1 = TSreadState(TOUCHfront), reading2 = TSreadState(TOUCHback);
...
/*	DisplayCenteredTextLine(2, "%d, %d", USreadDist(USfront),SensorValue(USback));
	...
	mecMove(15, 90, 0, 7);//move away from the wall
	...
	case 1:{
			...
			mecMove(80, 90, 0, 70);//move sideway
			mecMove(80, 0, 0, 100);//move forward
